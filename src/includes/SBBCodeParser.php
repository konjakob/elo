...
class SBBCodeParser_TextNode extends SBBCodeParser_Node
{
	...
	{
		if(!$nl2br)
			return str_replace("  ", " &nbsp;", $this->text);//htmlentities($this->text, ENT_QUOTES | ENT_IGNORE, "UTF-8"));
		
		return str_replace("  ", " &nbsp;", nl2br($this->text));//htmlentities($this->text, ENT_QUOTES | ENT_IGNORE, "UTF-8")));
	}
	...
	public static function default_bbcodes()
	{
		return array(
			...
			new SBBCodeParser_BBCode('youtube', function($content, $attribs)
			{
				...
				return '<iframe width="480" height="390" src="' . $uri . '" frameborder="0" allowfullscreen></iframe>';
